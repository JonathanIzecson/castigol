    ...
    <div>
        <div class="bg-gray-800 h-screen overflow-hidden flex items-center justify-center">
            <div class="bg-white border rounded-lg lg:w-5/12 md:6/12 w-10/12 shadow-3xl">
                <form class="p-12 md:p-24" @submit.prevent="initSession">
                    <div class="flex items-center text-lg mb-6 md:mb-8 bg-gray-200 border rounded-lg">
                        ...
                        <input type="text" id="username" v-model="form.login"
                    ...
                    <div class="flex items-center text-lg mb-6 md:mb-8 bg-gray-200 border rounded-lg">
                        ...
                        <input type="password" id="password" v-model="form.password"
                            class="bg-gray-200 pl-2 py-2 md:py-4 focus:outline-none w-full" placeholder="Password..." />
                    </div>
                    <button :disabled="emptyFields" :class="{ 'cursor-not-allowed': emptyFields }"
                        class="bg-gray-800 font-medium p-2 md:p-4 mb-8 text-white uppercase w-full hover:bg-gray-900 transition duration-500 ease-in-out">Iniciar
                        sesion</button>
                    <div v-if="error" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
                        <strong class="font-bold">Algo salió mal!</strong>
                        <span class="block sm:inline"> Usuario y/o contraseña incorrectos.</span>
                    </div>
                </form>
            </div>
        </div>
